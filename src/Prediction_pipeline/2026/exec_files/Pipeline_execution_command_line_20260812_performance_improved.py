...
def lookup_ols(scientific_name):
    url = "https://www.ebi.ac.uk/ols4/api/search"   # adjust if you use OLS3
    ...
    try:
        ...
        print(f"[OLS] Request failed for '{scientific_name}': {e}")
        return None
    except ValueError:                   # JSON decode error
        print(f"[OLS] Non-JSON response for '{scientific_name}'")
        return None

    # Use .get() so a missing key returns {} / 0 instead of raising
    ...
    if num_found > 0:
        ...
        if docs:
            return docs[0].get("iri")    # or the field you actually need
    return None
...
def run_scispacy_check(df, max_workers=10):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            name = futures[future]          # get the name from the mapping
            try:
                _, result = future.result()  # call result() ONCE
            except Exception as e:
                print(f"[Worker] Task failed for '{name}': {e}")
                taxonomy_cache[name] = (None, None, None)  # optional: record failure
                continue
...
#def llm_to_ask(df, model_id, previous_step_check, text_to_evaluate, prompt, client):
        ...
        if biobert_ok.startswith("No"):
            ...
            if "gpt" in model_id:
                response = client.chat.completions.create(
                    model=model_id,
                    messages=[
                        {"role": "system", "content": "This is your context: " + str(i[text_to_evaluate])},
                        {"role": "user",   "content": (
                            "Based on the context provided, give me the name of three scientific species "
                            "that could be used in this study. If you don't know, say 'I don't know'."
                        )},
                    ],
                )
                
                df.at[row, "GPT_description_combined_species"] = response.choices[0].message.content
                time.sleep(5)
                print(f"The row {row} has been processed.")

            else:
                print(f'No model has been selected.')
...
def llm_to_ask(df, model_id, previous_step_check, text_to_evaluate, prompt, client):
    ...
    for row, i in df.iterrows():
        ...
        if not biobert_ok.startswith("No"):
            print('The species has been identified in a previous step.')
            continue
        ...
        if "Llama" in model_id or "Mistral" in model_id:
            ...
            outputs = pipe(message, max_new_tokens=256)
            response = outputs[0]["generated_text"][-1]["content"]
            df.at[row, "Species_LLM"] = remove_special_characters(response).lower()
            print(f"Processed row {row} for species extraction.")
            ...
            df.at[row, "Species_LLM"] = remove_special_characters(content).lower()  # ← correct column
            time.sleep(5)
            print(f"The row {row} has been processed.")
    ...
    print(f"Execution time LLM: {time.time() - start_time:.2f} seconds")
    return df
...
def run_complete_pipeline(df, model_id, prompt, client, columns, value_to_compare):
    wk_df = combined_text_in_file(df,columns, "text_combined")
    ...
    df_scispacy, count_match_species_scispacy = matching_values(df_scispacy, value_to_compare, "Scispacy_vs_real_value", "Species_Scientific_Name_SciSpacy")

    df_biobert = run_biobert(df_scispacy, "Comparison_Scispacy_vs_real_value", "text_combined")
    df_biobert, count_match_species_biobert = matching_values(df_biobert,value_to_compare, "BioBert_vs_real_value", "Species_Scientific_Name_BioBERT")

    df_LLM = llm_to_ask(df_biobert, model_id, "Comparison_BioBert_vs_real_value", "text_combined", prompt, client)
    df_LLM, count_match_species_llm = matching_values(df_LLM, value_to_compare, "LLM_vs_real_value", "Species_LLM")

    return df_LLM, count_match_species_scispacy, count_match_species_biobert, count_match_species_llm
...
def main():
    ...
    name_database = sys.argv[2]

    with open(f'output_{name_database}.txt', 'w') as out_f:
        ...
        try:
            ...
            value_to_compare = config['value_to_compare']
            ...
            df_complete, count_match_species_scispacy, count_match_species_biobert, count_match_species_llm = run_complete_pipeline(wk_df, model_id, prompt,client, columns, value_to_compare)
